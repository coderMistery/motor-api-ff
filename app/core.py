...
# Certifique-se de que a pasta ff_proto está no mesmo nível da pasta app
...
async def get_access_token(client: httpx.AsyncClient, uid: str, password: str) -> Tuple[str, str]:
    # Extrai secret e id das configurações
...
async def create_jwt(uid: str, password: str) -> Dict[str, str]:
    # IMPORTANTE: http2=True ajuda a evitar detecção de bot
    async with httpx.AsyncClient(http2=True, follow_redirects=True) as client:
        ...
        # User-Agent atualizado para simular um dispositivo real
        ...
        if token == "0" or len(token) == 0:
            ...
            raise RuntimeError(f"Failed to obtain JWT. Response: {res_dict}")
